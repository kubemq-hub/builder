...
func (c *Connection) askResponseChannel() error {
	val := ""
	err := survey.NewInput().
		SetKind("string").
		SetName("response_channel").
		SetMessage("Sets source response channel").
		SetDefault(fmt.Sprintf("source.%s.%s", c.name, c.kind)).
		SetHelp("Sets source response channel").
		SetRequired(true).
		Render(&val)
	if err != nil {
		return err
	}
	c.properties["response_channel"] = val
	return nil
}
...
func (c *Connection) renderEventsKind() (map[string]string, error) {
	...
	if err := c.askResponseChannel(); err != nil {
		return nil, err
	}
