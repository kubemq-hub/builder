...
func (b *Binding) addSource(defaultName string) error {
	...
	if b.Source.Properties, err = connector.RenderProperties(b.loadedOptions); err != nil {
		return err
	}
	return nil
}
	...
	*ftProperties = fmt.Sprintf("<p> Edit Source Properties (%s)", edited.Source.Kind)
	...
	form.AddItem(ftKind, func() error {
		...
		if lastKind != edited.Source.Kind {
			...
			if edited.Source.Properties, err = connector.RenderProperties(edited.loadedOptions); err != nil {
				return err
			}
		}
		*ftKind = fmt.Sprintf("<k> Edit Source Kind (%s)", edited.Source.Kind)
...
func (b *Binding) addTarget(defaultName string) error {
	...
	if b.Target.Properties, err = connector.RenderProperties(b.loadedOptions); err != nil {
		return err
	}
	return nil
}
	...
	*ftProperties = fmt.Sprintf("<p> Edit Target Properties (%s)", edited.Target.Kind)
	...
	form.AddItem(ftKind, func() error {
		...
		if lastKind != edited.Target.Kind {
			...
			if edited.Target.Properties, err = connector.RenderProperties(edited.loadedOptions); err != nil {
				return err
			}
		}
		*ftKind = fmt.Sprintf("<k> Edit Target Kind (%s)", edited.Target.Kind)
...
func AddSourceIntegration(takenNames []string, connectorsManifest []byte, loadedOptions DefaultOptions) (*Binding, error) {
	...
	if b.Source.Properties, err = sources[b.Source.Kind].RenderProperties(b.loadedOptions); err != nil {
		return nil, err
	}
	...
	if b.Target.Properties, err = targets[b.Target.Kind].RenderProperties(b.loadedOptions); err != nil {
		return nil, err
	}
